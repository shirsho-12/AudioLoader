...
def check_md5(path, md5_hash):
    ...
    #     assert (
    #         md5_returned == md5_hash
    #     ), f"{os.path.basename(path)} is corrupted, please download it again"
    return True
...
class MultilingualLibriSpeech(Dataset):
    ...
    def __init__(
        ...
        if download:
            ...
            if os.path.isdir(download_path):
                ...
                decision = input(
                    f"Do you want to extract {language_name+ext_archive} again?\n"
            ...
            print(
                f"Splitting utterance labels, it might take a long time for large languages such as English."
            )
        ...
        if low_resource:
            if one_hr == False and isinstance(one_hr, bool):
                ...
                with open(
                    os.path.join(
                        self._path, "limited_supervision", "9hr", "handles.txt"
                    ),
                    "r",
                ) as f:
            ...
            if one_hr:
                raise ValueError(
                    f"When `low_resource`={low_resource}, "
                ...
                self._walker = sorted(
                    str(p.stem)
                    for p in Path(self._path).glob("audio/*/*/*" + self._ext_audio)
                )

        if refresh == True:
            if self.sample_rate:
                print(
                    f"Resampling audio files to {self.sample_rate}Hz. "
    ...
    def extract_limited_train_set(self):
        ...
        if os.path.exists(target_root):
            decision = input(
                f"{target_root} already exists, this function will overwrite existing files.\n"
        ...
        for i in tqdm.tqdm(unique_paths, desc="Creating `limited_train` set"):
            ...
            audio_path = os.path.join(
                audio_root, speaker_id, chapter_id, i + self._ext_audio
            )

            output_path = os.path.join(
                target_root, speaker_id, chapter_id, i + self._ext_audio
            )
        ...
        copy_tree(
            os.path.join(train_folder, "limited_supervision"),
            os.path.join(target_train_folder, "limited_supervision"),
        )
    ...
    def extract_labels(self, split_name, num_threads=0, Text=True, IPA=False):
        ...
        if IPA:
            ipa_labels = glob.glob(
                os.path.join(root, "audio", "*", "*", "*.ipa_trans.txt")
            )
            if len(ipa_labels) > 0:
                decision = input(
                    f"IPA labels `ipa_trans.txt` in {split_name} folder already exist,"
        ...
        if len(labels) > 0:
            decision = input(
                f"text labels `.trans.txt` in {split_name} folder already exist,"
        ...
        audio_list = glob.glob(
            os.path.join(root, "audio", "*", "*", f"*{self._ext_audio}")
        )

        if num_threads == 0:
            for audio_file in tqdm.tqdm(
                audio_list, desc=f"Extracting {split_name} set"
            ):
    ...
    def clear_cache(self):
        ...
        if len(cache_files) > 0:
            decision = input(
                f"{len(cache_files)} .pt files found, confirm clearing cache? [yes/no]"
            )
            if decision.lower() == "yes":
                ...
                raise ValueError(
                    f"Input {decision} is not recognized, please choose `yes` or `no`."
                )
        elif len(cache_files) == 0:
            print(f"No cache file found")
        else:
            raise ValueError(
                f"Something is wrong, why does there are {len(cache_files)} cache files"
            )
    ...
    def load_librispeech_item(
        ...
        processed_data_path = os.path.join(
            path, "audio", speaker_id, chapter_id, saved_name
        )
        ...
        if self.use_cache == True and os.path.isfile(processed_data_path):
            ...
            if (
                sample_rate != self.sample_rate and self.sample_rate != None
            ):  # If the sample_rate is above 16k, downsample it

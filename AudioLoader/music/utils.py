...
def process_csv(input_file, output_file):
    ...
    np.savetxt(
        output_file, csv_data, "%.6f", "\t", header="onset\toffset\tnote\tvelocity"
    )
        ...
        if input_file.endswith(".mid") or input_file.endswith(".csv"):
            if output_dir == False:
                ...
                output_file = os.path.join(
                    output_dir, os.path.basename(input_file[:-4]) + ".tsv"
                )
            ...
            if output_dir == False:
                ...
                output_file = os.path.join(
                    output_dir, os.path.basename(input_file[:-5]) + ".tsv"
                )
        else:
